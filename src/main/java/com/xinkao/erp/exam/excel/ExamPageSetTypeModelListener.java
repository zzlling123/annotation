...
public class ExamPageSetTypeModelListener extends AnalysisEventListener<ExamPageSetImportModel> {
    ...
    private Map<Integer, List<ExamPageSetType>> addExamPageSetPointMap = new HashMap<>();
    ...
    public void invoke(ExamPageSetImportModel examPageSetImportModel, AnalysisContext analysisContext) {
        ...
        //如果题库题数大于0，且抽取提数大于0，则开始添加主观题
        if(Integer.parseInt(choiceAnswerCount) > 0 && StrUtil.isNotBlank(choiceAnswerChouCount)){
            //如果抽取提数大于题库题数，则报错
            if (Integer.parseInt(choiceAnswerChouCount) > Integer.parseInt(choiceAnswerCount)){
                msg = "题目分类："+type+"，导入信息有误：主观题题抽取提数不能大于题库题数";
                errorList.add(getHandleMsg(rowNum + 1, msg));
                handleResult.setErrorList(errorList);
        ...
        //如果题库题数大于0，且抽取提数大于0，则开始添加操作题
        if(Integer.parseInt(choicePracticeCount) > 0 && StrUtil.isNotBlank(choicePracticeChouCount)){
            //如果抽取提数大于题库题数，则报错
            if (Integer.parseInt(choicePracticeChouCount) > Integer.parseInt(choicePracticeCount)){
                msg = "题目分类："+type+"，导入信息有误：操作题题抽取提数不能大于题库题数";
                errorList.add(getHandleMsg(rowNum + 1, msg));
                handleResult.setErrorList(errorList);
                ...
                if(Integer.parseInt(choicePracticeChouCount) > 0){
                    //添加操作题
                    ExamPageSetType examPageSetType = new ExamPageSetType();
                    examPageSetType.setExamId(examId);
                    examPageSetType.setTypeId(questionType.getId());
    ...
    private String checkEmptyError(ExamPageSetImportModel examPageSetImportModel, int row) {
        ...
        if (StrUtil.isNotBlank(choiceFillChouCount)) {
            if (!NumberUtil.isNumber(choiceFillChouCount)){
                msgList.add("填空题抽取题数必须为纯数字");
            }else {
                if (StrUtil.isBlank(choiceFillScore)) {
                    msgList.add("请填写填空题分值");
                }else {
                    if (!NumberUtil.isNumber(choiceFillScore)){
                        msgList.add("填空题分值必须为纯数字");
                    }
                }
            }
        }
        if (StrUtil.isNotBlank(choiceAnswerChouCount)) {
            if (!NumberUtil.isNumber(choiceAnswerChouCount)){
                msgList.add("问答题抽取题数必须为纯数字");
            }else {
                if (StrUtil.isBlank(choiceAnswerScore)) {
                    msgList.add("请填写问答题分值");
                }else {
                    if (!NumberUtil.isNumber(choiceAnswerScore)){
                        msgList.add("问答题分值必须为纯数字");
                    }
                }
            }
        }
        if (StrUtil.isNotBlank(choicePracticeChouCount)) {
            if (!NumberUtil.isNumber(choicePracticeChouCount)){
                msgList.add("实践题抽取题数必须为纯数字");
            }else {
                if (StrUtil.isBlank(choicePracticeScore)) {
                    msgList.add("请填写实践题分值");
                }else {
                    if (!NumberUtil.isNumber(choicePracticeScore)){
                        msgList.add("实践题分值必须为纯数字");
                    }
                }
            }
        }
    ...
    private void saveData() {
        ...
        examPageSetService.importExamPageSetType(response,addExamPageSetPointMap, handleResult,examPageSetImportErrorModelList,token);
        log.info("存储数据库成功！");
    }
